// ==UserScript==
// @name         Tongyi URL Query
// @namespace    http://tampermonkey.net/
// @version      1.0.5
// @description  Submit Tongyi prompts via a URL query parameter
// @author       kyleczhang
// @match        https://www.tongyi.com/*
// @icon         https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/qwen.png
// @license      MIT
...
(async () => {
  ...
  const isDisabled = (btn) => {
    ...
    const classList = btn.classList
      ...
      : btn.className.split(/\s+/);
    return (
      classList.some((cls) => cls.startsWith("disabled--")) ||
  ...
  const queryParam =
    sessionStorage.getItem("tongyi-query") ||
  ...
  const getSendButton = (scope = document) => {
    ...
    const iconUse = scope.querySelector(
      'use[href="#icon-fasong_default"], use[xlink\\:href="#icon-fasong_default"]',
    );
    if (iconUse) {
    ...
    const ariaSend = Array.from(scope.querySelectorAll("button, div")).find(
      (el) => {
        const label = (
  ...
  const inputWrapper =
    textarea.closest('div[class^="inputOutWrap--"]') || textarea.parentElement;
  const wrapperNode = inputWrapper || document;
  const sendBtn = await waitFor(() => getSendButton(wrapperNode), {
    timeout: maxWaitTime,
    scope: wrapperNode,
  });
  if (!sendBtn) {
  ...
  const observerRoot =
    wrapperNode.nodeType === Node.DOCUMENT_NODE
      ? wrapperNode.documentElement
      : wrapperNode;
